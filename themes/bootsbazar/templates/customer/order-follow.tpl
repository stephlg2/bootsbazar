      ...
      <tbody>
        {foreach from=$ordersReturn item=return}
          <tr>
            ...
            <td class="text-sm-center link-print-return">
              {if $return.print_url}
                <a href="{$return.print_url}">{l s='Print out RMA' d='Shop.Theme.Actions'}</a>
              {/if}
             
              {hook h='displayReturnLink' return=$return}
              <br />
              {if $proforma[$return.id]}
              <a href="{$proforma[$return.id]}">{l s="Facture Proforma" mod='ras_proforma_retours'}
                {/if}
            </td>
          </tr>
        {/foreach}
      ...
      {foreach from=$ordersReturn item=return}
        <div class="order-return">
          <ul>
            ...
            {if $return.print_url}
              <li  class="link-print-return_mobile">
                {if $return.print_url}
                  <a href="{$return.print_url}">{l s='Print out RMA' d='Shop.Theme.Actions'}</a>
                {/if}
