...
{block name='order_products_table'}
        ...
        <tr>
          <td>
              ...
              <a {if isset($product.download_link)}href="{$product.download_link}"{/if}>
                <span class="image image-historique-commande">
                <img src="{$product.cover.medium.url}" class="img-fluid"/>
              </span> 
              <p class="nom-produit-historique">• {$product.name}
              <span class="ref-produit-historique">• {if $product.reference}
  ...
  <div class="order-items visible--mobile box">
    {foreach from=$order.products item=product}
      <div class="order-item bloc-produit-historique">
        <div class="row">
          <div class="col-12 desc">
          
            <div class="name bloc-name-historique">
            <span class="image-historique-commande-mobile">
                <img src="{$product.cover.medium.url}" class="img-fluid"/>
              </span>
              <div class="nom-produit-historique-mobile">
              {$product.name}
              <span class="ref-historique-mobile">{if $product.reference}</span>
          ...
          <div class="col-sm-12 qty totaux-historique-mobile">
            
              <p class="text-sm-left text-left">
              <strong>{l s='Unit price' d='Shop.Theme.Catalog'}</strong> : {$product.price}
              </p>
              <p>
              <strong>{l s='Quantity' d='Shop.Theme.Catalog'}</strong> : {if $product.customizations}
                  {foreach $product.customizations as $customization}
                    {$customization.quantity}
                  {/foreach}
                {else}
              ...
              <p>
              <strong>{l s='Total price' d='Shop.Theme.Catalog'}</strong> : {$product.total}
              </p>
