  ...
  {foreach from=$groups key=id_attribute_group item=group}
      ...
      {elseif $group.group_type == 'color'}
          ...
          {foreach from=$group.attributes key=id_attribute item=group_attribute}
              ...
              {assign var="converted" value=$product.attributes[$id_attribute_group]['id_attribute']|intval}
              <label class="label-color {if $group['attributes_quantity'][$id_attribute] <=0}{hook h="displayStatusProduct" id_product=$product.id id_attribute=$id_attribute}{/if} {if $id_attribute == $converted }current-product{/if}">
                ...
                ><span class="tailles-shoes">{$group_attribute.name}</span></span>
              </label>
          {/foreach}
