            ...
            <div class="carousel_content owl-carousel">
                {foreach from=$manufacturers item=manufacturer}
                    <div class="hi-carousels-images {if !$carousel_options['activate_carousel']}on_left{/if}">
                        <a href="{$link->getManufacturerLink($manufacturer.id_manufacturer|escape:'htmlall':'UTF-8', $manufacturer.link_rewrite|escape:'htmlall':'UTF-8')}"
                        title="{$manufacturer.name|escape:'htmlall':'UTF-8'}" class="manufacturer_link">
                        <img src="{$manufacturer_img_dir|escape:'htmlall':'UTF-8'}{$manufacturer.id_manufacturer|escape:'htmlall':'UTF-8'}{if $mfc_img_size}-{$mfc_img_size|escape:'htmlall':'UTF-8'}{/if}.jpg" alt="{$manufacturer.name|escape:'htmlall':'UTF-8'}" />
                        </a>
                    </div>
                {/foreach}
            </div>
