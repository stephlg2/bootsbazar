...
{block name='order_products_table'}
  <form class="w-100 mw-100 form--100" id="order-return-form" action="{$urls.pages.order_follow}" method="post" novalidate autocomplete="false">
              ...
              <div class="thumbnail product-thumbnail">
                  {if $product.cover}
                      <img data-src="{$product.cover.bySize.home_default.url}"
                          ...
                          data-full-size-image-url="{$product.cover.category.url}" class="lazyload">
            ...
            <td class="qty">
              {if !$product.customizations}
                ...
                {if $product.quantity > $product.qty_returned}
                  <div class="select" id="_desktop_return_qty_{$product.id_order_detail}" {if $product.quantity-$product.qty_returned == 1} style="display: none" {/if}>
              ...
              {else}
                {foreach $product.customizations as $customization}
                  ...
                  <div class="select" id="_desktop_return_qty_{$product.id_order_detail}_{$customization.id_customization}" {if $product.quantity-$product.qty_returned == 1} style="display: none" {/if}>
                    <select
    ...
    <div class="order-items visible--mobile box">
      {foreach from=$order.products item=product}
        <div class="order-item">
          <div class="">
            <div class="checkbox select-produit-mobile">
             {if !$product.customizations}
              ...
              {else}
                ...
                {/foreach}
              {/if}
              <span class="select-return-mobile">{l s='Select this product' d='Shop.Theme.Catalog'}</span> 
            </div>
            <div class="content">
              <div class="row">
                <div class="col-sm-12 desc tab-mobile">
                ...
                <div class="col-sm-12 qty qty-mobile">
                  <div class="row">
                  <div class="col-12 tab-mobile tab-ref-mobile">
                  {if $product.reference}
                    <div class="ref">{l s='Reference' d='Shop.Theme.Catalog'}: <span>{$product.reference}</span></div>
                  {/if}
                  </div>
                    <div class="col-12 unit-price-mobile tab-mobile">
                    {l s='Unit price' d='Shop.Theme.Catalog'}: <span>{$product.price}</span>
                    </div>
                    <div class="col-12 col-quantite-mobile tab-mobile">
                      {if $product.customizations}
                        {foreach $product.customizations as $customization}
                          <div class="q">{l s='Quantity' d='Shop.Theme.Catalog'}: <span>{$customization.quantity}</span></div>
                    ...
                    <div class="col-12 total-price-mobile tab-mobile">
                    {l s='Total price' d='Shop.Theme.Catalog'}: <span>{$product.total}</span>
                    </div>
                  </div>
                </div>
              </div>
    ...
    <div class="box bloc-retour-order-detail">
      <header>
      ...
      <section class="form-fields">
        <div class="form-group">
          {if $motifs}
          <label for="id_return_msg">{l s='Motif' d='Shop.Forms.Labels'}</label>
            <select id="id_return_msg" name="returnText" class="custom-select" required>
              <option value="0">{l s='-- please choose --' d='Shop.Forms.Labels'}</option>
              {assign var=optgroup value=false}
              {foreach from=$motifs item=motif}
                {if $motif.groupe}
                  {if $optgroup} </optgroup> {/if}
                  <optgroup label="{$motif.lib}">
                  {$optgroup = true}
                {else}
                <option value="{$motif.value}">{$motif.lib}</option>
                {/if}
              {/foreach}
              {if $optgroup} </optgroup> {/if}
            </select>
