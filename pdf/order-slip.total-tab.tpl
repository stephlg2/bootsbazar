...
{assign var=taux_tva value=(($order->total_paid_tax_incl / ($order->total_paid_tax_excl)) -1 ) * 100}
{assign var=coef_tva value=$order->total_paid_tax_incl / $order->total_paid_tax_excl}
{if $total_cart_rule }
    {assign var=totalTTC value=$order->total_paid_tax_incl - $total_cart_rule}
    {assign var=totalHT value=$totalTTC / $coef_tva}
{else}
    {assign var=totalTTC value=$order->total_paid_tax_incl}
    {assign var=totalHT value=$order->total_paid_tax_excl}
{/if}
...
<table id="total-tab" width="100%">
    ...
    {if isset($order_details) && count($order_details) > 0}
        <tr>
            ...
            <td class="white" width="30%">
                - {displayPrice currency=$order->id_currency price=$totalTTC}
            </td>
        </tr>
        <tr>
            <td class="grey" width="70%">
            ...
            <td class="white" width="30%">
                - {displayPrice currency=$order->id_currency price=$totalHT}
            </td>
        </tr>
    {/if}
    ...
    {if ($order->total_paid_tax_incl - $order->total_paid_tax_excl) > 0}
        <tr>
            <td class="grey" width="70%">
                {l s='Total Tax' d='Shop.Pdf' pdf='true'} {* <span style="font-size:9px">({$taux_tva|string_format:"%.2f"}  %)</span> *}
            </td>
            <td class="white" width="30%">
                - {displayPrice currency=$order->id_currency price=($totalTTC - $totalHT)}
            </td>
        </tr>
    {/if}
    ...
    {if $amount_choosen}
        <tr class="bold">
            <td class="grey" width="70%">
                {l s='Total discount' d='Shop.Pdf' pdf='true'}
            </td>
            <td class="white" width="30%">

                + {displayPrice currency=$order->id_currency price=($totalTTC - $order_slip->amount+$order_slip->shipping_cost_amount)}
